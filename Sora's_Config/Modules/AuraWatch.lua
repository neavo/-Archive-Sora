...
function C.AuraWatch.LoadSettings()
	...
	for key, value in pairs(Default["ALL"]) do
		local Flag = true
		for k, v in pairs(AuraWatchDB) do
			if value["Name"] == v["Name"] then Flag = false end
		end
		if Flag then tinsert(AuraWatchDB, value) end
	end
