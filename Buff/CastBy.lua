   ...
   if caster then
      ...
      if UnitIsPlayer(caster) then
         ...
         if caster == 'pet' or caster == 'vehicle' then
            ...
            castername = format('|cff%02X%02X%02X%s|r%s%s', color.r*255, color.g*255, color.b*255, UnitName('player'), '|cffC0C0C0µÄ|r', UnitName(caster))
         else
            partypet = caster:match('^partypet(%d+)$')
            raidpet = caster:match('^raidpet(%d+)$')
            if partypet then
               color = colors[select(2,UnitClass('party'..partypet))]
               castername = format('|cff%02X%02X%02X%s|r%s%s' , color.r*255, color.g*255, color.b*255, UnitName("party"..partypet), '|cffC0C0C0µÄ|r', UnitName(caster))
            elseif raidpet then
               color = colors[select(2,UnitClass('party'..raidpet))]
               castername = format('|cff%02X%02X%02X%s|r%s%s' , color.r*255, color.g*255, color.b*255, UnitName('raid'..raidpet), '|cffC0C0C0µÄ|r', UnitName(caster))
            end
         end
      end
      self:AddDoubleLine('|cffC0C0C0Cast by:|r', castername)
      self:Show()
   end
end
