...
local Module = Sora:NewModule("BossTargetFrame")
...
local function BuildBossTargetFrame(self, ...)
	-- RegisterForClicks
	self.menu = BuildMenu
...
function Module:OnInitialize()
	if not (UnitFrameDB.ShowBossTarget and UnitFrameDB.ShowBoss) then return end
	oUF:RegisterStyle("SoraBossTarget", BuildBossTargetFrame)
	oUF:SetActiveStyle("SoraBossTarget")
	DB.BossTargetFrame = {}
	for i = 1, MAX_BOSS_FRAMES do
		DB.BossTargetFrame[i] = oUF:Spawn("bosstarget"..i)
		DB.BossTargetFrame[i]:SetPoint("BOTTOMLEFT", DB.BossFrame[i], "TOPLEFT", 0, 10)
	end
end