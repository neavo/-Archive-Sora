...
local lb = CreateFrame("Button", "m_LootAdv", addon)		-- Link button
...
local print = function(a) ChatFrame1:ALDDessage("|cff33ff99m_Loot:|r "..a) end
local iconsize = 28
local sq, ss, sn
...
local createSlot = function(id)
	...
	frame:SetPoint("LEFT", iconsize + 10, 0)
	...
	iconFrame:SetPoint("LEFT", frame, -iconsize,0)
	...
	name:SetFont(DB.Font, 11, "OUTLINE")

	name:SetWidth(120)
	frame.name = name
	
	local drop = frame:CreateTexture(nil, "ARTWORK")
	drop:SetTexture(bartex)
	...
	drop:SetAlpha(.5)
	frame.drop = drop
	frame:SetPoint("TOP", addon, 8, (-5+iconsize)-(id*(iconsize+10))-10)
...
lb:SetHeight(10)
lb:ClearAllPoints()
lb:SetPoint("TOP", addon, "TOP", 0, -5)
lb:SetFrameStrata("TOOLTIP")
lb:RegisterForClicks("RightButtonUp", "LeftButtonUp")
lb:SetScript("OnClick", OnLinkClick)
lb:SetScript( "OnLeave", function() lb.text:SetTextColor(1, 1, 1) end )
lb:SetScript( "OnEnter", function() lb.text:SetTextColor(0.7, 0.2, 0.2) end )
lb:Hide()
UIDropDownMenu_Initialize(LDD, LDD_Initialize, "MENU")

lb.text = lb:CreateFontString(nil, "OVERLAY")
lb.text:SetPoint("CENTER", lb, "CENTER", 0, 0)
lb.text:SetFont("Fonts\\ZYKai_T.ttf", 18, "THINOUTLINE")
lb.text:SetText("∨")
...
addon.LOOT_OPENED = function(self, event, autoloot)
	...
	local close = CreateFrame("Button",nil,addon)
	close:SetPoint("TOPRIGHT", addon, "TOPRIGHT", 0, -3)
	close:SetWidth(20)
	close:SetHeight(14)
	close:SetScript( "OnClick", function(self) self:GetParent():Hide() end)
	close:SetScript( "OnLeave", function() close.Text:SetTextColor(1, 1, 1) end )
	close:SetScript( "OnEnter", function() close.Text:SetTextColor(0.7, 0.2, 0.2) end )
	close.Text = close:CreateFontString(nil, "OVERLAY")
	close.Text:SetPoint("CENTER", close, "CENTER", 0, 1)
	close.Text:SetFont("Fonts\\ZYKai_T.ttf", 15, "THINOUTLINE")
