...
local function style(frame)
    ...
    if frame.NumLines then
        for index=1, frame:NumLines() do
            if index == 1 then
                _G[frame:GetName()..'TextLeft'..index]:SetFont(DB.Font, 10+2, "THINOUTLINE")
            else
                _G[frame:GetName()..'TextLeft'..index]:SetFont(DB.Font, 10, "THINOUTLINE")
            end
            _G[frame:GetName()..'TextRight'..index]:SetFont(DB.Font, 10, "THINOUTLINE")
        end
    end
end
