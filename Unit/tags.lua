...
oUF.Tags['karma:info'] = function(u)
    local _, class = UnitClass(u)
...
oUF.TagEvents['karma:info'] = 'UNIT_HEALTH UNIT_CONNECTION'
...
oUF.Tags['karma:pp'] = function(u)
	...
	if str and power > 0 then
		if powerMax >130 then
			return SVal(power).." | "..math.floor(power/powerMax*100+.5).."%"
		else
			return SVal(power)
		end
	end
end
oUF.TagEvents['karma:pp'] = 'UNIT_POWER'
