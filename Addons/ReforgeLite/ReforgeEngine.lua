...
function ReforgeLite:GetMethodScore (method)
  ...
  if method.tankingModel then
    ...
    if method.tankingModel == "WARRIOR" then
    ...
    elseif method.tankingModel == "PALADIN" then
...
function ReforgeLite:UpdateMethodStats (method)
  ...
  if method.tankingModel then
    ...
    if method.stats.block and 5 + method.stats.dodge + method.stats.parry + method.stats.block > unhit then
      method.stats.block = unhit - 5 - method.stats.dodge - method.stats.parry
    end
  end
end
...
function ReforgeLite:GetItemReforgeOptionsTank (item, data, slot)
  ...
  if data.method.tankingModel ~= "DRUID" then
...
function ReforgeLite:InitReforgeTank ()
  ...
  local data = {}
  data.method = method
  if method.tankingModel == "WARRIOR" or method.tankingModel == "PALADIN" then
    data.weights = {}
    for i = 1, #self.itemStats do
      data.weights[i] = 0
    end
  ...
  if method.tankingModel ~= "DRUID" then
...
function ReforgeLite:ChooseReforgeTank (data, reforgeOptions, scores, codes)
  ...
  for k, score in pairs (scores) do
    ...
    if data.method.tankingModel == "WARRIOR" then
      ...
      if 5 + dodge + parry + block > data.unhit then
        block = data.unhit - 5 - dodge - parry
      end
    ...
    elseif data.method.tankingModel == "PALADIN" then
      local mastery = data.init.mastery + score
      local block = 5 + (8 + mastery / self:RatingPerPoint (self.STATS.MASTERY)) * 2.25
      if 5 + dodge + parry + block > data.unhit then
        block = data.unhit - 5 - dodge - parry
      end
    ...
    if bestCode == nil or score > bestScore then
      bestCode = code
      bestScore = score
    end
  end
