...
function mod:OnCombatStart(delay)
	...
	if self:GetUnitCreatureId("target") == 45872 or self:GetUnitCreatureId("focus") == 45872 or not self.Options.OnlyWarnforMyTarget then--Rohash
		...
		sndEast:Schedule(26-delay, "Interface\\AddOns\\DBM-Core\\extrasounds\\windblastsoon.mp3")
	...
	elseif args:IsSpellID(86193) then
		...
		if self:GetUnitCreatureId("target") == 45872 or self:GetUnitCreatureId("focus") == 45872 or not self.Options.OnlyWarnforMyTarget then--Rohash
			...
			if windBlastCounter == 1 then
				...
				sndEast:Schedule(78, "Interface\\AddOns\\DBM-Core\\extrasounds\\windblastsoon.mp3")
			else
				timerWindBlastCD:Start()
				sndEast:Schedule(56, "Interface\\AddOns\\DBM-Core\\extrasounds\\windblastsoon.mp3")
			end
		end
	end
end
