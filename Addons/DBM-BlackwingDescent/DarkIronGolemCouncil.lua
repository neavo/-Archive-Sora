...
local bossActivate = function(boss)
	if boss == L.Magmatron or boss == 42178 then
		if not mod:IsHealer() then
			sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\attackfire.mp3")
		end
		incinerateCast = 0
		timerAcquiringTarget:Start(20)--These are same on heroic and normal
		sndWOP:Schedule(17, "Interface\\AddOns\\DBM-Core\\extrasounds\\targetsoon.mp3")
	...
	elseif boss == L.Electron or boss == 42179 then
		if not mod:IsHealer() then
			sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\attacklightning.mp3")
		end
		if mod:IsDifficulty("heroic10", "heroic25") then
			timerLightningConductorCD:Start(15)--Probably also has a variation if it's like normal. Needs more logs to verify.
			sndWOP:Schedule(12, "Interface\\AddOns\\DBM-Core\\extrasounds\\eleaesoon.mp3")
			...
			sndWOP:Schedule(9, "Interface\\AddOns\\DBM-Core\\extrasounds\\eleaesoon.mp3")
			warnUnstableShieldSoon:Schedule(40)
		end
	elseif boss == L.Toxitron or boss == 42180 then
		if not mod:IsHealer() then
			sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\attackpoison.mp3")
		end
		if mod:IsDifficulty("heroic10", "heroic25") then
	...
	elseif boss == L.Arcanotron or boss == 42166 then
		if not mod:IsHealer() then
			sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\attackarcane.mp3")
		end
...
local bossInactive = function(boss)
	if boss == L.Magmatron then
		...
		sndWOP:Cancel("Interface\\AddOns\\DBM-Core\\extrasounds\\targetsoon.mp3")
	...
	elseif boss == L.Electron then
		...
		sndWOP:Cancel("Interface\\AddOns\\DBM-Core\\extrasounds\\eleaesoon.mp3")
...
function mod:SPELL_AURA_APPLIED(args)
	...
	elseif args:IsSpellID(79501, 92035, 92036, 92037) then
		...
		if self:IsDifficulty("heroic10", "heroic25") then
			...
			sndWOP:Schedule(24, "Interface\\AddOns\\DBM-Core\\extrasounds\\targetsoon.mp3")
		else
			timerAcquiringTarget:Start()
			sndWOP:Schedule(37, "Interface\\AddOns\\DBM-Core\\extrasounds\\targetsoon.mp3")
		end
		if args:IsPlayer() then
			...
			sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\avoidtarget.mp3")
		end
...
--			soundLightningConductor:Play()
		...
		if self:IsDifficulty("heroic10", "heroic25") then
			...
			sndWOP:Schedule(17, "Interface\\AddOns\\DBM-Core\\extrasounds\\eleaesoon.mp3")
		else
			timerLightningConductor:Start(args.destName)
			timerLightningConductorCD:Start()
			sndWOP:Schedule(22, "Interface\\AddOns\\DBM-Core\\extrasounds\\eleaesoon.mp3")
		end
...
--			soundFixate:Play()
	...
	elseif args:IsSpellID(79629, 91555, 91556, 91557) and args:IsDestTypeHostile() then--Check if Generator buff is gained by a hostile.
		...
		if args:GetDestCreatureID() == targetCID and args:GetDestCreatureID() ~= 42897 then--If target gaining buff is target then not an ooze (only hostiles left filtering oozes is golems)
			...
			if mod:IsTank() then
				sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\bossout.mp3")
			end
		end
	elseif args:IsSpellID(92048) then--Shadow Infusion, debuff 5 seconds before shadow conductor.
		...
		sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\shadowae.mp3")
